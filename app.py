...
HELP_MESSAGE = (
    ...
    "③ 運行情報・遅延の確認\n"
    "   「運行情報」「遅延」と送信すると、現在の南北線の運行状況を確認できます。\n\n"
    "④ 位置情報から最寄り駅を検索\n"
...
def get_origin_train_numbers(target_station_id: str) -> set:
    ...
    origin_set = set()
    try:
        res = requests.get(ODPT_TRAIN_TIMETABLE_URL, params=params, timeout=5)
...
def build_timetable_message(station_info: dict = None, target_time_str: str = None, direction_key: str = "MEGURO") -> str:
    ...
    # 1. 運行情報をチェック（遅延・運転見合わせ発生時のみ警告ヘッダーを表示）
    ...
    try:
        res = requests.get(ODPT_STATION_TIMETABLE_URL, params=params_st, timeout=8)
